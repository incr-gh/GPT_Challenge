...
def send_request(prompt, ref_image=None, image_paths=[], temperature=0.2):
    ...
    response = requests.post(
        "https://api.openai.com/v1/chat/completions", headers=HEADERS, json=payload, timeout=3.5).json()
    # print(content)
    return response
    ...
    for problem in questions[CURRENT:]:
        ...
        while MAX_RETRIES>0 and failed:
            try:
                ...
                if random.random() <.7:
                    raise "sad"
                response = send_request(prompt, diagram, image_links, temperature=0.43+0.1*(3-MAX_RETRIES))
                ...
                print(ans)
                with open(r"D:\Projects\AMIC AI Challenge\AI_CHALLENGE_sample\results\one_shot_fix.txt", 'a') as f:
        ...
        if failed:
            with open(r"D:\Projects\AMIC AI Challenge\AI_CHALLENGE_sample\results\one_shot_fix.txt", 'a') as f:
                    f.write(f'{problem["id"]} {random.choice("abcde")} {time.time()-t1}\n') 
...
for q in questions:
    di[q['id']]=q
print(len(di.keys()))
with open(r'D:\Projects\AMIC AI Challenge\AI_CHALLENGE_sample\results\one_shot.txt','r') as f:
    ans= f.readlines()
    ans= list(map(lambda x: x.split()[0], list(filter(lambda x: x.split()[1]=="FAILED", ans))))

new_problems=[]
for a in ans:
    new_problems.append(di[int(a)])
